...
コース一覧画面において、当初はページ番号ベースのページング（下部のページボタン）を実装していましたが、データの追加や更新が発生した際に、表示内容の重複や抜けが発生する問題がありました。

-【原因】

ページ番号ベースの方式では、データの変動によって取得範囲がずれ、ユーザーが意図しない結果を受け取る可能性がありました。

-【解決】

問題の改善方法を検討する中で、チームメンバーが以前に実装したスクロールベースのページング機能を提案してくれました。
その仕組みを理解した上で自分の機能に適用し、データの取得基準を固定することで、重複や欠損のない安定したページング処理を実現しました。

-【結果】

スクロールベースのページングに変更したことで、データの整合性が保たれるようになり、重複表示や欠損の問題を解消することができました。
また、ユーザーが自然に次のデータを閲覧できるようになり、UXの向上にもつながりました。
